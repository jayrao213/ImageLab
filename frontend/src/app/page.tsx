...
export default function Page() {
  ...
  }, [action, r, g, b, amount, factor, size, degrees, block, prompt])
  ...
  }, [file, action, r, g, b, amount, factor, size, degrees, block, prompt, revoke])

  const reset = useCallback(() => {
    currentBlob.current = file
    revoke()
    setPreviewUrl(null)
  }, [file, revoke])
    ...
    <main className="mx-auto max-w-5xl p-4 font-sans">
      ...
      <section className={`${sectionCls} mb-4`}> 
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button
            onClick={apply}
            disabled={isBusy || (!file && !currentBlob.current) || (action === 'ai_generate' && !prompt.trim())}
