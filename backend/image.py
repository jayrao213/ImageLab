...
class Image:
    ...
    def ai_generate(self, prompt: str, width: int = 512, height: int = 512):
        """
        Generate an image using Pollinations.ai text-to-image API
        """
        if not prompt:
        ...
        try:
            ...
            pil_out = PILImage.open(io.BytesIO(response.content)).convert("RGB")
            ...
            raise RuntimeError(f"Failed to generate image with Pollinations.ai: {str(e)}")
